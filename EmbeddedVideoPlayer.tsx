 ...
 */
export const getCleanEmbedUrl = (url: string): { type: 'youtube' | 'vimeo' | 'html5' | 'iframe'; embedUrl: string } => {
  if (!url || !url.trim()) {
  ...
  if (ytWatchMatch && ytWatchMatch[1]) {
    ...
    // Use youtube-nocookie with parameters that prevent external link popups and cross-origin postMessage errors
    return {
      type: 'youtube',
      embedUrl: `https://www.youtube-nocookie.com/embed/${videoId}?rel=0&modestbranding=1&playsinline=1&controls=1`,
    };
  }
  ...
  if (vimeoMatch && vimeoMatch[1]) {
    ...
    return {
      type: 'vimeo',
      embedUrl: `https://player.vimeo.com/video/${vimeoId}?playsinline=1&title=0&byline=0&portrait=0`,
    };
  }
...
export const EmbeddedVideoPlayer: React.FC<EmbeddedVideoPlayerProps> = ({
  ...
  return (
    <div className={`relative w-full h-full min-h-[300px] bg-[#000000] rounded-xl overflow-hidden group/player select-none flex flex-col justify-between ${className}`}>
