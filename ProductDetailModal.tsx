...
export const ProductDetailModal: React.FC<ProductDetailModalProps> = ({
  ...
  const [showVideo, setShowVideo] = useState<boolean>(
    Boolean(initialShowVideo && product?.videoUrl) ||
    Boolean(product?.mediaType === 'video' && product?.videoUrl)
  );
  ...
  // Track legitimate Meta ViewContent event on modal open
  useEffect(() => {
    if (product) {
      trackViewContent({
        id: product.id,
  ...
  // Ensure background videos pause on open, and all videos pause on close
  useEffect(() => {
  ...
  // When switching away from video to photos, pause and mute
  useEffect(() => {
    if (!showVideo) {
      pauseAndMuteAllVideos();
    }
