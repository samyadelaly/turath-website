...
export const TurathLogo: React.FC<TurathLogoProps> = ({
  ...
  return (
    <div className={`relative inline-flex items-center gap-3 select-none group/logo ${className}`}>
      {/* Raster Logo Graphic - True rectangular proportions (1200x896) */}
      <div className={`relative overflow-hidden rounded-md border border-[#d4c59d]/40 bg-[#000000] shadow-[0_2px_20px_rgba(212,197,157,0.15)] flex items-center justify-center p-0.5 sm:p-1 transition-all duration-300 group-hover/logo:border-[#d4c59d] ${dimensions[size]}`}>
        <img
          src={logoSrc || DEFAULT_LOGO_URL}
          alt="TURATH - Handcrafted Egyptian Brass, Copper & Decorative Metals Official Logo"
          className="w-full h-full object-contain transition-transform duration-300 group-hover/logo:scale-[1.02]"
          onError={(e) => {
