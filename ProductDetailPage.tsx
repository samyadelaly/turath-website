...
export const ProductDetailPage: React.FC<ProductDetailPageProps> = ({
  ...
  useEffect(() => {
    ...
    metaDesc.setAttribute('content', productDesc);
    ...
    // 5. Schema.org JSON-LD for rich Product structured data (no fake prices or fake offers)
    const hasRealPrice = Boolean(product.price && !isNaN(parseFloat(product.price)) && parseFloat(product.price) > 0);
    const productSchema: Record<string, any> = {
      '@context': 'https://schema.org/',
      '@type': 'Product',
      name: product.nameEN || product.name,
      image: validImages.length > 0 ? validImages : [absoluteImage],
    ...
    if (product.material || product.materials) {
      productSchema.material = product.material || product.materials;
    }
    ...
    return () => {
      ...
      // Revert or clean dynamic Open Graph tags
  ...
  }, [product, validImages, activeImage, category]);

  // Video & audio coordination: silence other videos when entering or switching product, and when unmounting
  useEffect(() => {
  ...
  // When switching from video tab to photos tab, pause and mute
  useEffect(() => {
    if (!showVideo) {
      pauseAndMuteAllVideos();
    }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 sm:pt-8 lg:pt-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-14">
          ...
          <div className="lg:col-span-7 space-y-4">
            ...
            {/* Main Stage Display with Dynamic Aspect Ratio & Fit Mode */}
            {(() => {
              const mainRatio = computeProductImageRatio(product, activeImage);
              const isDisplayingVideo = showVideo && Boolean(product.videoUrl);
              const videoAspect = computeProductMediaRatio(product, 'video');

              return (
                <div 
                  className="relative w-full max-w-[560px] max-h-[82vh] mx-auto rounded-xl overflow-hidden bg-[#0c0c0f] border border-[#d4c59d]/40 flex items-center justify-center group shadow-[0_10px_35px_rgba(0,0,0,0.8)]"
                  style={{ 
                    aspectRatio: isDisplayingVideo 
                      ? (videoAspect.isOriginal ? undefined : videoAspect.aspectRatioCss)
                      : (mainRatio.isOriginal ? undefined : mainRatio.aspectRatioCss)
                  }}
                >
                  {isDisplayingVideo ? (
                    <TurathMedia
                      ...
                      poster={product.videoPoster || activeImage}
                    ...
                    <TurathMedia
                      ...
                      containerClassName="w-full h-full cursor-pointer bg-[#0c0c0f]"
                    ...
                    >
                      ...
                      <button
                        ...
                        className="absolute top-3 right-3 p-2 rounded-lg bg-black/70 text-[#d4c59d] hover:bg-black hover:text-[#f5f0e6] transition-colors backdrop-blur-sm z-10"
              ...
              <div className="flex items-center justify-center gap-2.5 overflow-x-auto pb-2 custom-scrollbar">
                {validImages.map((img, idx) => {
                  ...
                  <button
                    ...
                    className={`relative w-14 rounded-lg overflow-hidden flex-shrink-0 transition-all border ${
                ...
                {product.videoUrl && (
                  <button
                    onClick={() => setShowVideo(true)}
                    className={`relative w-14 aspect-[9/16] rounded-lg overflow-hidden flex-shrink-0 transition-all border flex flex-col items-center justify-center gap-1 ${
                      showVideo
        ...
        <div className="mt-12 sm:mt-14 lg:mt-16 pt-8 sm:pt-10 border-t border-[#d4c59d]/30 space-y-10 sm:space-y-12">
            ...
            <div className="space-y-6 pt-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {relatedProducts.map((rel) => {
                  const relRatio = computeProductImageRatio(rel);
                  return (
                  <div
                    key={rel.id}
                  ...
                  >
                    <TurathImage
                      ...
                      computedRatio={relRatio}
                      containerClassName="w-full bg-black"
                      imageClassName="transition-transform duration-500 group-hover:scale-105"
                    >
                      <span className="absolute top-2 left-2 px-2 py-0.5 rounded bg-black/80 text-[10px] font-mono text-[#d4c59d] z-10">
                        {rel.sku || rel.id}
                      </span>
                      {rel.imageRatio && rel.imageRatio !== 'Original' && (
                        <span className="absolute top-2 right-2 px-1.5 py-0.5 rounded bg-black/80 text-[9px] font-mono text-[#d4c59d] z-10">
                          {rel.imageRatio}
                        </span>
                      )}
                    </TurathImage>
        ...
        {isZoomOpen && (
          ...
          >
            ...
            <TurathImage
              ...
              ratio="Original"
              fit="contain"
              containerClassName="max-w-full max-h-[90vh] rounded-lg shadow-2xl border border-[#d4c59d]/40"
